    ...
    def connect(self, **kwargs):
        ...
        try:
            ...
            if self.engine.startswith('sqlite'):
                self.connection = self.connector.connect(kwargs['host'] + kwargs['database'])
                self.table_schema = None
...
""" + table_layout + """
    ...
    def check_attributes(self, attributes_lod, action=None):
        ...
        if self.db_object.connection == None:
            print 'Error checking attributes of table:', self.name, 'because no connection exists to database', self.db_object.name
            return
        ...
        if self.name not in table_list:
            if action == 'add' or action == 'convert' or action == 'replace':
        ...
        for attributes_dic in attributes_lod:
            ...
            if column_name not in column_list:
                differences_lod.append(attributes_dic)

        # Is there any difference?
        if action <> 'analyze':
            if len(differences_lod) > 0:
                for attributes_dic in differences_lod:
                    new_column = column(self, attributes_dic['column_name'])
                    new_column.create(attributes_dic)

        # Are the given column attributes matching those in the database?
        # attributes_not_in_table, attributes_not_in_definition
        # ---------------------------------------------------------------------
        # differences_lod can be cool that way:
        source_difference_lod = [{}]
        target_difference_lod = [{}]
        #dummy_lod = [{'difference': 'attribute', 'filament': 0}]
    ...
    def get_content(self):
        ...
        content_lod = self.db_object.dictresult("SELECT * FROM " + self.name)
        return content_lod
...
# Bullshit, this is not possible because if one table is referenced multiple
# times from the same table, this fails!
#    
#    def join_all_references(self, attributes_lod):
#        join_str = ''
#        print "attributes_lod:", attributes_lod
#        for attributes_dic in attributes_lod:
#            if attributes_dic.has_key('referenced_table_name'):
#                if attributes_dic.has_key('referenced_column_name'):
#                    join_str += """\
#LEFT JOIN
#    %(referenced_table_name)s ON %(referenced_table_name)s.%(referenced_column_name)s = """ % attributes_dic + self.name + """.%(column_name)s
#""" % attributes_dic
#        sql_command = """\
#SELECT * FROM """ + self.name + "\n" + join_str
#        print sql_command
#        selection = self.db_object.dictresult(sql_command)
#        return selection
    ...
    def select(self, distinct=False, column_list=[], where=''):
        return self.db_object.select(self.name, distinct, column_list, where)
            ...
            if primary_key_column <> '':
                ...
                for column_name in column_list:
                    if content_dict.has_key(column_name):
                        column_names_list += '    %s,\n' % column_name
                        column_content = content_dict[column_name]
                        if column_content == None:
                            column_content = 'NULL'
                        elif type(column_content) == str:
                            column_content = "'%s'" % column_content
    ...
    def delete(self, primary_key_column='', primary_key=None):
        sql_command = 'DELETE FROM %s WHERE %s = %s' % (self.name, primary_key_column, primary_key)
        try:
            self.db_object.execute(sql_command)
        except:
            raise
    ...
    def get_attribute_layout(self, attributes_dic = None):
        ...
        if attributes_dic.has_key('referenced_table_name'):
            if attributes_dic.has_key('referenced_column_name'):
                ...
                referenced_table.check_attributes(referenced_table_attributes, action='add')
