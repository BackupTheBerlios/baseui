...
def makeNSI(pathname, build_dir, app_name, app_version):
    # Make installer with NSIS ------------------------------------------------
    nsi_file = open("%s\\build\\%s.nsi" % (pathname, app_name), 'w')
    nsi_file.write(get_nsi(APP_NAME=app_name, APP_VERSION=app_version))
    nsi_file.close()
