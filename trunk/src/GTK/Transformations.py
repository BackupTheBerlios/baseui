...
def merge_lod_fields(content_lod, merge_field_list, column_name, separator=', '):
    ...
    print 'merge_lod_fields is deprecated!'
    
    output_lod = []
    for content in content_lod:
        merged_content = ''
        for merge_field in merge_field_list:
            if content.has_key(merge_field):
                if merged_content == '':
                    merged_content += content[merge_field]
                else:
                    merged_content += '%s%s' % (separator, content[merge_field])
        content[column_name] = merged_content
        output_lod.append(content)
    return output_lod
