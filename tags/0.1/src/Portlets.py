...
from GTK import Containers, Dialogs, DataViews, Buttons, Glade
    ...
    def populate(self, content_lod=None):        
        if self.parent_form <> None and self.parent_form.primary_key <> None:
            ...
            self.foreign_key_column = \
                self.db_table.get_foreign_key_column(attributes_lod, 
                                                     primary_key_column, 
                                                     referenced_table_name)
            
            self.content_lod = self.db_table.select(where='%s = %i' % (self.foreign_key_column, self.parent_form.primary_key))
        else:
...
SELECT * FROM %s WHERE %s = %s''' % (self.db_table.name, self.primary_key_column, self.primary_key))
        ...
        for dic in self.definition_lod:
            if dic.has_key('populate_function'):
                if dic.has_key('widget_object'):
                    dic['populate_function'](dic['widget_object'])
    ...
    def save_dataset(self):
        try:
            ...
            # First, save the parent form (and hope that no error occurs!)
            if self.parent_form <> None:
