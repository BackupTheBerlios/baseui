...
class Sun:
    ...
    def decimal_to_hms(self, decimal_time, altzone=0):
        ...
        second = int(decimal_second)
        return hour, minute, second
    
    
    def hms_to_decimal(self, hour, minute, second):
        decimal_second = float(second) / 6000
        decimal_minute = float(minute) / 60
        
        decimal_time = hour + decimal_minute + decimal_second
        return decimal_time 
...
if __name__ == "__main__":
    ...
    k = Sun()
    sunrise, sunset = k.sunRiseSet(year, month, day, lon, lat)
    print "sunrise at:", k.decimal_to_hms(sunrise)
    print "sunset at: ", k.decimal_to_hms(sunset)

    print; print "unit conversion:"

    h,m,s = k.decimal_to_hms(sunrise)
    print sunrise, hms_to_decimal(h,m,s)
    h,m,s = k.decimal_to_hms(sunset)
    print sunset, hms_to_decimal(h,m,s)
