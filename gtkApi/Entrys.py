...
class Base:
    ...
    def connect(self, signal, function):
        self.widget.connect(signal, function)
...
class Table:
    ...
    def create(self, definition_lod):
        ...
        number_of_rows = len(self.definition_lod)
        self.widget = gtk.Table(rows=number_of_rows, columns=2)

        for row_number in xrange(number_of_rows):
            definition_dict = self.definition_lod[row_number]

            label = gtk.Label(definition_dict['label'])
            label.set_alignment(xalign=0, yalign=0.5)
            label.set_padding(4, 4)
            definition_dict['label_widget'] = label
            
            widget = definition_dict['widget']
...
class Combobox(Base):
    ...
    # Events ------------------------------------------------------------------
    def on_entry_changed(self, widget=None, data=None):
        ...
        ''' This function initializes database entrys for automatic validation
            ...
            definition_dic = Defines the Entry attributes.
                             {'column_name': 'person.name',
                              ...
                              'mask': %(lastname)s, %(firstname)s
                                  => this tells how the content is shown in the entry and its list!
                             }
    ...
    def populate(self, content_lod=[]):
        ...
        for content_tuple in enumerate(self.content_lod):
            ...
            if type(test_value) == dict:
                # Look, if there are multiple columns given (list or string).
                if type(column_name) == list:
                    if self.definition_dic.has_key('mask'):
                        mask = self.definition_dic['mask']
                        
                        content_dic = {}
                        for column in column_name:
                            content_dic[column] = str(content[column])
                        content_str = mask % content_dic
                else:
                    # If not, pass the content trough...
                    if content[column_name] == None:
                        continue
                    content_str = str(content[column_name])
            else:
                if content_tuple[1] == None:
                    continue
                content_str = str(content)
                
            liststore.append([id, content_str])
        ...
        self.widget.child.connect('changed', self.on_entry_changed)
