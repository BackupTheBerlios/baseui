...
class UserLogin:
    def __init__(self, content_lod=[], merge_field_list=[], parent=None):
        ...
        self.login_dialog = Portlets.Login()
        ...
        self.entry_password = self.login_dialog.entry_password
        
        if content_lod <> []:
            self.populate(content_lod, merge_field_list)
        self.person = None
...
class ftpLogin:
    ''' Multifunctional portlet which reads a ftp configuration
        from an ini-File and connects it. '''
    ...
    def __init__(self, ini_filename='', autosave=False, parent=None):
        ...
        self.ftp_dialog = Portlets.FTP()
        self.DialogBox = Dialogs.Simple(parent=self.parent)
        self.ini_file = FileSystem.iniFile(self.ini_filename)
        self.portlet = self.ftp_dialog.create()
        
        self.config_dic = self.get_settings_from_ini()
        self.ftp_dialog.populate(self.config_dic)

        self.ftp_dialog.connect_order = self.connect
        self.ftp_dialog.disconnect_order = self.disconnect


    # Foreign actions ---------------------------------------------------------
    def set_connect_function(self, on_connect):
        self.ftp_dialog.on_connect = on_connect


    def set_disconnect_function(self, on_disconnect):
        self.ftp_dialog.on_disconnect = on_disconnect


    def get_settings_from_ftp(self, ftp):
        self.ftp = ftp
        if self.ftp <> None:
            self.config_dic = database.config
        else:
            self.ftp_dialog.set_disconnected()
            return

        if self.ftp.connection <> None:
            self.ftp_dialog.set_connected()
        else:
            self.ftp_dialog.set_disconnected()
        return self.config_dic
    ...
    def get_settings_from_ini(self):
        try:
            self.config_dic = self.ini_file.dictresult('ftp')
            return self.config_dic
        except Exception, inst:
            error_text = """\
Während des Einlesens der FTP-Konfiguration ist folgender Fehler aufgetreten:
...
Soll die Konfigurationsdatei neu erstellt werden?""" % str(inst)
    ...
    def save_settings_to_ini(self):
        self.config_dic = self.ftp_dialog.get_config_dic()
        ini_text = """\
[ftp]
hostname = %(hostname)s
username = %(username)s
...
""" % self.config_dic
    ...
    def connect(self):
        try:
            self.config_dic = self.ftp_dialog.get_config_dic()
            self.ftp = FileTransfer.FTP()
            self.ftp.connect(hostname=self.config_dic.get('hostname', ''),
                             username=self.config_dic.get('username', ''),
...
class Serial:
    def __init__(self):
        self.com_ports = [0, 1, 2, 3, 4, 5, 6, 7]
